...
function SWEP:PlayerHitUtil(owner, damage, hitent, dmginfo)
    if dmginfo:GetDamage() > 0 and hitent:IsPlayer() and hitent:IsValid() then
        ...
        if bleed and bleed:IsValid() then
            bleed:AddDamage(math.Clamp(damage*owner:GetStamina()/100,0,40))
            bleed.Damager = owner
        else
            local stat = owner:GiveStatus("bleed")
            if stat and stat:IsValid() then
                stat:SetDamage(math.Clamp(damage*owner:GetStamina()/100,0,40))
                stat.Damager = owner
            end
        end
    end
...
function SWEP:PostHitUtil(owner, hitent, dmginfo, tr, vel)
    if hitent:IsPlayer() and hitent:IsValid() then
        local bleed = hitent:GetStatus("bleed")
        if bleed and bleed:IsValid() then
            local steal = math.min(bleed:GetDamage(),30)
            bleed = math.max(bleed - steal,0)
            bleed:AddDamage(-steal)
		end
	end
